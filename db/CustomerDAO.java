...
public class CustomerDAO {
    ...
    public List<Customer> findCustomersByType(boolean isCorporate) throws SQLException {
        String sql = "SELECT * FROM customers WHERE is_corporate = ? ORDER BY created_at DESC";
        List<Customer> customers = new ArrayList<>();

        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {

            stmt.setBoolean(1, isCorporate);
    ...
    private Customer mapResultSetToCustomer(ResultSet rs) throws SQLException {
        ...
        if (isCorporate) {
            ...
            return new Customer(companyName, address) {
                @Override
            ...
            return new Customer(firstName, surname, address, localDateOfBirth) {
                @Override
                public String getCustomerId() { return customerId; }
            };
        }
